...
function doGet(e) {
  ...
  try {
    ...
    else if (action === "bootstrap")      result = bootstrap(e.parameter.userId||"", e.parameter.role||"", e.parameter.lite === "1");
    ...
    else if (action === "getPrintAllowedUsers")     result = getPrintAllowedUsers();
    else if (action === "setPrintAllowedUsers")     result = setPrintAllowedUsers(body.userIds, body.actingRole||"");
...
function setPrintAllowedUsers(userIds, actingRole) {
  if (actingRole !== "admin") return {ok:false, error:"Only admin can manage this list"};
...
function getSorCodes() {
  return {ok:true, codes: tryParse(getConfigValue_("sorCodes", "{}"), {})};
}
function setSorCode(version, code, actingRole) {
  if (actingRole !== "admin") return {ok:false, error:"Only admin can set SOR codes"};
  var map = tryParse(getConfigValue_("sorCodes", "{}"), {});
  map[version] = code;
  setConfigValue_("sorCodes", JSON.stringify(map));
  return {ok:true, codes: map};
}
...
function bootstrap(userId, role, lite) {
  // "lite" powers the FIRST load: it returns only what the Home page needs.
  // T-Codes, Processes and the Non-SOR catalog are fetched later, on demand,
  // by the pages that actually use them.
  var sorV = getSORVersions();
  var estimates = getEstimates(userId, role);
  var rates = getChargeRates();
  var nonSor = lite ? null : getNonSOR();
  var tcodes = lite ? null : getTCodes();
  var procs  = lite ? null : getProcesses();
  var result = {
    ok: true,
    lite: !!lite,
    sorVersions: sorV.ok ? sorV.versions : [],
    sorVersionCounts: sorV.ok ? sorV.counts : {},
    nonSor: (nonSor && nonSor.ok) ? nonSor.items : [],
    tcodes: (tcodes && tcodes.ok) ? tcodes.items : [],
    processes: (procs && procs.ok) ? procs.items : [],
